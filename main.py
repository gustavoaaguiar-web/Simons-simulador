...
# --- CONFIGURACIÓN APP ---
st.set_page_config(page_title="Simons GG v14.0", page_icon="🦅", layout="wide")
...
try:
    ...
    ahora_arg_dt = datetime.now() # Fallback
    ...
    if os.path.exists(ARCHIVO_ESTADO):
        try:
            with open(ARCHIVO_ESTADO, "r") as f:
                data = json.load(f)
                # Si el saldo no coincide y no hay posiciones, reseteamos al saldo objetivo
                if not data.get("pos"):
                    data["saldo"] = SALDO_INICIAL
                return data
        except: pass
    return {"saldo": SALDO_INICIAL, "pos": {}, "notificados": []}
...
if ccl_m and mercado_abierto:
    for _, row in df_m.iterrows():
        ...
        if (desvio >= 0.005 or panico_sell) and activo in st.session_state.pos:
            ...
            enviar_alerta_operacion(f"🦅 VENTA: {activo}", f"Resultado: AR$ {valor_final - info_c['m']:,.2f}", f"s_{activo}_{ahora_arg_dt.minute}")
            guardar_estado()
            st.rerun()
        ...
        if puedo_comprar_auto and desvio <= -0.005 and row['Clima'] == "🟢" and activo not in st.session_state.pos:
            ...
            if st.session_state.saldo >= monto_op:
                ...
                enviar_alerta_operacion(f"🦅 COMPRA: {activo}", f"Precio: ${row['ARS']}", f"b_{activo}_{ahora_arg_dt.minute}")
                guardar_estado()
                st.rerun()

# --- INTERFAZ (ASEGURAR RENDERIZADO) ---
st.title("🦅 Simons GG v14.0 🤑")
if not es_dia_habil: estado_txt = "🔴 FIN DE SEMANA"
elif ahora_arg_time < hora_apertura: estado_txt = "🔴 CERRADO (Abre 11:00)"
...
if ccl_m:
    ...
    st.error("No se pudieron obtener datos del mercado. Reintentando...")

# --- BARRA LATERAL ---
with st.sidebar:
    st.header("📂 Cartera")
    st.write(f"**Exposición:** 12.50%")
    ...
    if st.session_state.pos:
        for t, info in list(st.session_state.pos.items()):
            ...
            p_act = p_act_fila[0] if len(p_act_fila) > 0 and p_act_fila[0] > 0 else info['p']
            v_act = (info['m'] / info['p']) * p_act
            gan_ars = v_act - info['m']
            with st.expander(f"📦 {t}", expanded=True):
                st.write(f"Ganancia: AR$ {gan_ars:,.2f}")
                if st.button(f"🔴 Vender {t}", key=f"man_{t}"):
                    st.session_state.saldo += v_act
